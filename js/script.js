...
'use strict';	
	...
	var slide_image_position = function(){
		$('#main-slider .slide-image').each(function(){
			...
			// }
			var small_image = $(this).data("small-image");
			var medium_image = $(this).data("medium-image");
			var large_image = $(this).data("large-image");

			var winWidth = $(window).width(); // store the window width

			if (winWidth <= 480 && small_image != "") {
				$(this).css("background-image", "url('" + small_image + "')");
			}
			else if (winWidth <= 991 && medium_image != "") {
				$(this).css("background-image", "url('" + medium_image + "')");
			}
			else if (winWidth > 991 && large_image != "") {
				$(this).css("background-image", "url('" + large_image + "')");
			}
		});
	}
